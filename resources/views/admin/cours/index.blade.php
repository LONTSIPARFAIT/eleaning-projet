...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-orange-100 dark:bg-gradient-to-br dark:from-gray-900 dark:to-gray-800 py-8 px-4 sm:px-6 lg:px-8">
    <div class="container mx-auto">
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 scroll-reveal">
            <h1 class="text-3xl font-bold text-blue-800 dark:text-orange-400 animate-fade-in-down flex items-center gap-2">
            ...
            <a href="{{ route('cours.create') }}" class="mt-4 md:mt-0 bg-red-500 hover:bg-red-600 dark:bg-orange-500 dark:hover:bg-orange-600 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300 ease-in-out flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                Créer un nouveau cours
            </a>
        </div>

        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6 scroll-reveal border border-blue-200 dark:border-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse ($cours as $cour)
                            <tr class="hover:bg-orange-50 dark:hover:bg-gray-700 transition duration-200">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ Str::limit($cour->description, 50) }}</td>
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $cour->duration }} minutes</td>
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ number_format($cour->price, 2) }} €</td>
                                <td class="px-4 py-3 text-sm">
                                    <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                                        <a href="{{ route('cours.show', $cour) }}" class="bg-blue-500 hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md text-sm transition duration-200 flex items-center gap-1">
                                        ...
                                        <a href="{{ route('cours.edit', $cour) }}" class="bg-orange-500 hover:bg-orange-600 dark:bg-orange-600 dark:hover:bg-orange-700 text-white font-bold py-2 px-4 rounded-md text-sm transition duration-200 flex items-center gap-1">
                                        ...
                                        <form action="{{ route('cours.destroy', $cour) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 font-semibold transition duration-200 flex items-center gap-1" onclick="confirmDelete(event, this)">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-3 text-center text-gray-600 dark:text-gray-300">Aucun cours disponible.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="mt-6 flex justify-center scroll-reveal">
                <a href="{{ route('dashboard') }}" class="bg-orange-500 hover:bg-orange-600 dark:bg-orange-600 dark:hover:bg-orange-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300 ease-in-out flex items-center gap-2">
