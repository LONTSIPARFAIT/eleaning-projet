...
@section('content')
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8 bg-gradient-to-br from-indigo-50 to-teal-50" x-data="{ openCourse: null }">
    <!-- Header avec animation d'entrée -->
    <div class="animate-fade-in-down bg-white rounded-lg shadow-md p-6 mb-6">
        <h1 class="text-2xl sm:text-3xl font-bold mb-4 text-indigo-900">Tableau de Bord de l'Étudiant</h1>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            @else
                @foreach ($cours as $cour)
                    <div
                        class="bg-indigo-100 shadow-lg rounded-xl p-4 sm:p-6 transform transition-all duration-300 hover:scale-105 hover:shadow-xl animate-fade-in-up"
                    ...
                    >
                        ...
                        <a
                            ...
                            class="mt-4 inline-block bg-teal-600 hover:bg-teal-700 text-white font-semibold py-2 px-4 rounded-lg transition-all duration-200 hover:shadow-md"
                        >
                            Voir le Cours
                        </a>
                    </div>
    ...
    <section class="mt-8">
        ...
        <div class="bg-indigo-100 shadow-lg rounded-xl p-4 sm:p-6 animate-fade-in-up">
            {{-- @if($assignments->isEmpty()) --}}
                {{-- <p class="mt-4 text-gray-500">Aucun devoir à rendre pour le moment.</p> --}}
            {{-- @else --}}
                <ul class="space-y-3">
                    {{-- @foreach ($assignments as $assignment) --}}
                        <li class="border-b border-teal-200 py-2 flex flex-col sm:flex-row sm:items-center sm:justify-between transition-all duration-300 hover:bg-teal-50">
                            {{-- {{ $assignment->title }} --}}
        ...
        <div class="bg-indigo-100 shadow-lg rounded-xl p-4 sm:p-6 animate-fade-in-up">
            <ul class="space-y-3">
                {{-- @foreach ($resources as $resource) --}}
                    <li class="border-b border-teal-200 py-2 transition-all duration-300 hover:bg-teal-50">
                        {{-- {{ $resource->link }} --}}
                        <a
                            href=""
                            class="text-teal-600 hover:text-teal-700 hover:underline transition-colors duration-200"
                        >
                            {{-- {{ $resource->title }} --}}qsqs
                        </a>
                    </li>
